...
const MODE_GUIDANCE: Record<ReadingMode, string> = {
  natal: "Read the natal chart only. Focus on enduring temperament, life patterns, nodes, angles, and fixed stars. Do not interpret current transit rows as part of this reading.",
...
export async function generateInterpretation(chart: ChartResult, mode: ReadingMode = "combined") {
  ...
  const reading = await ask([
    { role: "system", content: `${COSMOLOGY}\n${MODE_GUIDANCE[mode]}\nCreate a Markdown reading with exactly these top-level headings: ## Core Identity, ## Mind, ## Soul / Emotional Life, ## Spirit / Meaning, ## Key Placements, ## Fixed Stars, ## Transit Story, ## Career & Work, ## Relationships, ## Home & Family, ## Destiny & Life Arc, ## Synthesis, ## Mirror.\n\n${PSYCHOLOGICAL_TEMPLATE}\n\nThe Transit Story must receive the same depth and narrative care as a major natal placement. Begin by naming the supplied transit moment and current location, then tell the story of each strongest supplied transit-to-natal contact: what the transit planet symbolizes, what natal factor it touches, what inner chapter or pressure may be coming forward now, what old protective strategy may be activated, how that could show up in ordinary scenes, and what mature response is available. Explain the arc as a season of development—not a prediction of events. Include a trigger → reaction → short-term payoff → long-term cost → conscious choice sequence, a relationship or work example, a recognition moment, and a brief fatherly counsel for each major transit theme. Distinguish fast passing weather from slower developmental pressure when the supplied data supports that distinction, but do not invent duration or exact timing. Never claim that a transit causes an event, guarantees an outcome, or proves a psychological fact. Say what evidence would make the transit interpretation not fit.\n\nUse the supplied transit layer as a present-tense chapter, not as fortune-telling. Do not invent transits, dates, timing claims, or events. The whole reading should feel like one connected portrait from a guardian-guide who sees the user's dignity and potential while refusing to hide the truth. Let themes echo across sections, but do not repeat the same paragraph. Use headings as chapters in a story. Where appropriate, let the voice offer a short fatherly piece of counsel: what to practice, what to stop tolerating, what responsibility to accept, what temptation to resist, or what kind of person to become. Advice must arise from the chart evidence and be framed as an invitation, not an order. The Mirror should read like an honest closing scene: identify the recurring arc, emotional defenses, control/trust patterns, relationship feedback loops, self-defeating tendencies, strengths, and the gap between potential and habitual behavior. Tell the user what they may need to hear, not merely what feels good to hear. Pair every hard observation with context, compassion, and a concrete way to recognize or work with it. Include uncertainty and counter-evidence inside the prose with phrases such as “This is more likely if…” and “This may not fit if…”. Do not use bullet lists for the main interpretation except for a brief final set of recognition questions or practical experiments. Do not flatter, diagnose, manipulate, or make deterministic predictions.` },
