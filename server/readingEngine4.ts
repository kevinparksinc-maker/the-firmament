...
import type { PatternAnalysis, RawAspect, PatternScore, PlanetaryWar } from './patternEngine';
...
const NAKSHATRA_DATA: Record<string, {
  ...
  Ashwini:          { archetype: "The Healer",      ruler: "Ketu",    gana: "Deva",      shakti: "swift healing action" },
  Bharani:          { archetype: "The Restrainer",  ruler: "Venus",   gana: "Manushya",  shakti: "holding and transforming" },
  Krittika:         { archetype: "The Cutter",      ruler: "Sun",     gana: "Rakshasa",  shakti: "sharp discernment" },
  Rohini:           { archetype: "The Grower",      ruler: "Moon",    gana: "Manushya",  shakti: "creation and fertility" },
  Mrigashira:       { archetype: "The Seeker",      ruler: "Mars",    gana: "Deva",      shakti: "wandering and searching" },
  Ardra:            { archetype: "The Storm",       ruler: "Rahu",    gana: "Manushya",  shakti: "tearing to rebuild" },
  Punarvasu:        { archetype: "The Returner",    ruler: "Jupiter", gana: "Deva",      shakti: "coming back renewed" },
  Pushya:           { archetype: "The Nourisher",   ruler: "Saturn",  gana: "Deva",      shakti: "sustaining and protecting" },
  Ashlesha:         { archetype: "The Coiler",      ruler: "Mercury", gana: "Rakshasa",  shakti: "winding intensity" },
  Magha:            { archetype: "The Ancestor",    ruler: "Ketu",    gana: "Rakshasa",  shakti: "legacy and inheritance" },
  PurvaPhalguni:    { archetype: "The Lover",       ruler: "Venus",   gana: "Manushya",  shakti: "pleasure and play" },
  UttaraPhalguni:   { archetype: "The Supporter",   ruler: "Sun",     gana: "Manushya",  shakti: "steady partnership" },
  Hasta:            { archetype: "The Hand",        ruler: "Moon",    gana: "Deva",      shakti: "skill and manifestation" },
  Chitra:           { archetype: "The Artist",      ruler: "Mars",    gana: "Rakshasa",  shakti: "brilliant projection" },
  Swati:            { archetype: "The Wind",        ruler: "Rahu",    gana: "Deva",      shakti: "movement and independence" },
  Vishakha:         { archetype: "The Forker",      ruler: "Jupiter", gana: "Rakshasa",  shakti: "splitting to choose" },
  Anuradha:         { archetype: "The Friend",      ruler: "Saturn",  gana: "Deva",      shakti: "loyalty and follow-through" },
  Jyeshtha:         { archetype: "The Elder",       ruler: "Mercury", gana: "Rakshasa",  shakti: "senior wisdom" },
  Mula:             { archetype: "The Root",        ruler: "Ketu",    gana: "Rakshasa",  shakti: "uprooting to origin" },
  PurvaAshadha:     { archetype: "The Purifier",    ruler: "Venus",   gana: "Manushya",  shakti: "cleansing flow" },
  UttaraAshadha:    { archetype: "The Victor",      ruler: "Sun",     gana: "Manushya",  shakti: "unyielding win" },
  Shravana:         { archetype: "The Listener",    ruler: "Moon",    gana: "Manushya",  shakti: "hearing truth" },
  Dhanishtha:       { archetype: "The Drummer",     ruler: "Mars",    gana: "Rakshasa",  shakti: "rhythmic power" },
  Shatabhisha:      { archetype: "The Veiler",      ruler: "Rahu",    gana: "Rakshasa",  shakti: "hidden healing" },
  PurvaBhadrapada:  { archetype: "The Burner",      ruler: "Jupiter", gana: "Manushya",  shakti: "sacrificial fire" },
  UttaraBhadrapada: { archetype: "The Depths",      ruler: "Saturn",  gana: "Manushya",  shakti: "deep stability" },
  Revati:           { archetype: "The Traveler",    ruler: "Mercury", gana: "Deva",      shakti: "journey's end" }
};
...
const TRANSIT_PRESSURE: Record<string, {
  hard: string;
  conj: string;
  soft: string;
}> = {
...
function getTransitPressureText(transitPlanet: string, aspectType: string): string {
  const pressure = TRANSIT_PRESSURE[transitPlanet];
  if (!pressure) return `${transitPlanet} influences this nakshatra's expression.`;
...
function buildStarLine(conj: StarConjunction, mode: ReadingMode): string {
  ...
  const royalTag = star.isRoyal ? ' [ROYAL STAR — WATCHER]' : star.isPolar ? ' [POLE STAR]' : '';
...
export type QuestionArea = 'career' | 'relationships' | 'health' | 'finance' | 'spirituality' | 'general' | 'creativity' | 'home';
...
function selectPillarPlanet(
  ...
  return available.reduce((best, cur) => {
    const bestScore = best.rawStrength + (best.isCazimi ? 0.2 : 0) - (best.isCombust ? 0.15 : 0);
    const curScore  = cur.rawStrength  + (cur.isCazimi  ? 0.2 : 0) - (cur.isCombust  ? 0.15 : 0);
...
function buildPillarRead(
  ...
  const relevantAspects = aspects
    .filter(a => a.planet1 === planetInput.planet || a.planet2 === planetInput.planet)
    .sort((a, b) => b.strength - a.strength);

  const topAspect = relevantAspects[0] || null;
  const transitingPlanet = topAspect
    ? (topAspect.planet1 === planetInput.planet ? topAspect.planet2 : topAspect.planet1)
    : null;
  const transitNote = topAspect && transitingPlanet
  ...
  // Fixed star lines
  const starLines = planetStars.length > 0
    ? '\n' + planetStars.map(c => buildStarLine(c, mode)).join('\n')
    : '';

  const synthesis = `${opening} ${signLine}${intensityTag}${rxTag}${transitLine}${starLines}`.trim();
...
function getHardAspectTruth(aspect: RawAspect, mode: ReadingMode): { truth: string; why: string } {
...
function getWarTruth(war: PlanetaryWar, mode: ReadingMode): string {
  if (mode === 'direct') return `${war.winner} overpowers ${war.loser} within 1°. ${war.winner}'s expression dominates. Work with the winner.`;
  if (mode === 'soul')   return `${war.winner} and ${war.loser} are in close quarters. ${war.winner} is louder. Listen there first.`;
  return `Planetary war: ${war.winner} > ${war.loser} (${war.degreeDiff.toFixed(2)}° separation). ${war.winner} sets the tone.`;
}

function getStrengthNote(planet: string, strength: number, mode: ReadingMode): string | null {
  if (strength > 0.8) {
    if (mode === 'direct') return `${planet} is dominant. Its voice overrides others in the chart right now.`;
    if (mode === 'soul')   return `${planet} energy is amplified. What it represents cannot be sidestepped.`;
    return `${planet}: ${Math.round(strength * 100)}% strength. Dominant.`;
  }
  if (strength < 0.3) {
    if (mode === 'direct') return `${planet} is weakened. Its expression is muted or distorted.`;
    if (mode === 'soul')   return `${planet} energy is quiet right now. You may not feel its influence clearly.`;
...
function getActionForPattern(pattern: PatternScore, mode: ReadingMode): string {
  ...
  return actions[pattern.type]?.[mode] || `Attend to ${pattern.planets.join(', ')}. This pattern requires conscious engagement.`;
}

function getWarningForPattern(pattern: PatternScore, mode: ReadingMode): string | null {
...
export function generateReading(
  ...
  if (context.planetData) {
    ...
    if (pd['Moon']) {
      ...
      pillars.push(buildPillarRead('Soul', pd['Moon'], moonDegree, analysis.aspects, mode, starConjunctions));
    ...
    if (mindPlanet) {
      ...
      pillars.push(buildPillarRead('Mind', mindPlanet, mindDegree, analysis.aspects, mode, starConjunctions));
    ...
    if (spiritPlanet) {
      ...
      pillars.push(buildPillarRead('Spirit', spiritPlanet, spiritDegree, analysis.aspects, mode, starConjunctions));
  ...
  const starNotes: string[] = starConjunctions
    ...
    .map(c => {
      const royalTag = c.star.isRoyal ? ' [ROYAL STAR]' : c.star.isPolar ? ' [POLE STAR]' : '';
  ...
  const sortedStrengths = Object.entries(analysis.strengths)
    .sort((a, b) => b[1].rawStrength - a[1].rawStrength);
  ...
  if (context.focusArea && mode === 'direct') {
    actions.push(`Your question about ${context.focusArea} maps directly onto these patterns. Act accordingly.`);
  }
  ...
  return {
    ...
    pillars,
    truth:     truths.join(' '),
    why:       whys.slice(0, 2).join(' ') || 'The mechanics are straightforward. Planets interact. Patterns emerge.',
    action:    actions.slice(0, 2).join(' ') || 'No single action is indicated. Maintain awareness.',
...
export function formatReading(reading: ReadingOutput, context: UserContext): string {
  const modeLabel =
    reading.mode === 'direct'  ? '⚡ DIRECT TRUTH' :
    reading.mode === 'soul'    ? '🌙 SOUL READING' :
                                 '🔄 INTEGRATED';
...
`;
  ...
  for (const pillar of reading.pillars) {
    const starHeader = pillar.starConjunctions.length > 0
...
export function quickReading(
  ...
  if (question) {
    ...
    if (q.includes('fix') || q.includes('stop') || q.includes('how do i'))         mode = 'direct';
    if (q.includes('feel') || q.includes('soul') || q.includes('why am i'))        mode = 'soul';
    if (q.includes('career') || q.includes('work') || q.includes('job'))           focusArea = 'career';
    if (q.includes('love') || q.includes('partner') || q.includes('relationship')) focusArea = 'relationships';
    if (q.includes('money') || q.includes('finance') || q.includes('income'))      focusArea = 'finance';
    if (q.includes('health') || q.includes('body') || q.includes('sick'))          focusArea = 'health';
    if (q.includes('spirit') || q.includes('purpose') || q.includes('dharma'))     focusArea = 'spirituality';
  }

  return generateReading(analysis, { question, readingMode: mode, focusArea, planetData, starConjunctions });
}