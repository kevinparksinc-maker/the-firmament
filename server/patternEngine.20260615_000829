...
export interface PatternScore {
  ...
  signs: string[];
  description: string;
  type: 'stellium' | 'grand_trine' | 't_square' | 'rahu_ketu_axis' | 'parivartana' | 'planetary_war';
}
...
const EXALTATIONS: Record<string, string> = {
  ...
  Uranus: 'Scorpio', Neptune: 'Aquarius', Pluto: 'Leo'
};

const DEBILITATIONS: Record<string, string> = {
  Sun: 'Libra', Moon: 'Scorpio', Mercury: 'Pisces', Venus: 'Virgo',
  Mars: 'Cancer', Jupiter: 'Capricorn', Saturn: 'Aries',
  Uranus: 'Taurus', Neptune: 'Leo', Pluto: 'Aquarius'
};
...
export function calculateRawStrength(planet: string, sign: string, rx: boolean): PlanetaryStrength {
  let rawStrength = 0.5;
  ...
  return { planet, rawStrength, isExalted, isDebilitated, isInOwnSign, isRetrograde: rx };
}
...
export function detectAspects(
  ...
  const aspectDefs = [
    { type: 'conjunction' as const, angle: 0, orb: config.orbConjunction, isHard: true, isSoft: false },
    { type: 'opposition' as const, angle: 180, orb: config.orbOpposition, isHard: true, isSoft: false },
    { type: 'square' as const, angle: 90, orb: config.orbSquare, isHard: true, isSoft: false },
    { type: 'trine' as const, angle: 120, orb: config.orbTrine, isHard: false, isSoft: true },
    { type: 'sextile' as const, angle: 60, orb: config.orbSextile, isHard: false, isSoft: true },
    { type: 'quincunx' as const, angle: 150, orb: config.orbQuincunx, isHard: true, isSoft: false }
  ];
...
export function detectStellium(planets: Record<string, PlanetPlacement>): PatternScore | null {
...
export function detectGrandTrine(aspects: RawAspect[], planets: Record<string, PlanetPlacement>): PatternScore | null {
  ...
  for (let i = 0; i < trines.length; i++) {
    for (let j = i + 1; j < trines.length; j++) {
      ...
      const planetsSet = new Set([t1.planet1, t1.planet2, t2.planet1, t2.planet2]);
...
export function detectTSquare(aspects: RawAspect[], planets: Record<string, PlanetPlacement>): PatternScore | null {
  const squares = aspects.filter(a => a.type === 'square' && a.strength > 0.5);
  const oppositions = aspects.filter(a => a.type === 'opposition' && a.strength > 0.5);
  
  for (const opp of oppositions) {
    const p1 = opp.planet1;
    const p2 = opp.planet2;
    
    const squareToP1 = squares.find(s => 
      (s.planet1 === p1 || s.planet2 === p1) && 
      s.planet1 !== p2 && s.planet2 !== p2
    );
    
    const squareToP2 = squares.find(s => 
      (s.planet1 === p2 || s.planet2 === p2) && 
      s.planet1 !== p1 && s.planet2 !== p1
    );
    
    if (squareToP1 && squareToP2) {
      const thirdPlanet = squareToP1.planet1 === p1 ? squareToP1.planet2 : squareToP1.planet1;
      if (thirdPlanet === (squareToP2.planet1 === p2 ? squareToP2.planet2 : squareToP2.planet1)) {
        return {
          name: `T-Square: ${p1}–${p2} opposition squared by ${thirdPlanet}`,
          strength: (opp.strength + squareToP1.strength + squareToP2.strength) / 3,
          planets: [p1, p2, thirdPlanet],
          houses: [p1, p2, thirdPlanet].map(p => planets[p]?.house || 0).filter(h => h > 0),
...
export function detectRahuKetuAxis(planets: Record<string, PlanetPlacement>): PatternScore | null {
  const rahu = planets['Rahu'] || planets['NorthNode'];
  const ketu = planets['Ketu'] || planets['SouthNode'];
  
  if (!rahu || !ketu || rahu.absolute === null || ketu.absolute === null) return null;
...
export function detectParivartana(planets: Record<string, PlanetPlacement>): PatternScore | null {
  const planetList = Object.entries(planets);
...
export function detectPlanetaryWar(planets: Record<string, PlanetPlacement>): PlanetaryWar[] {
  const wars: PlanetaryWar[] = [];
  const planetList = Object.entries(planets);
  
  const PRIORITY: Record<string, number> = {
    Saturn: 5, Jupiter: 4, Pluto: 3.8, Rahu: 3.5, Ketu: 3.5, Mars: 3,
    Uranus: 3.2, Neptune: 3.1, Sun: 2, Venus: 2, Mercury: 2, Moon: 1.5
  };
...
export function detectHouseClusters(planets: Record<string, PlanetPlacement>): Record<number, string[]> {
...
export function analyzePatterns(
  ...
  if (stellium && stellium.strength >= config.minimalPatternStrength) patterns.push(stellium);
  
  const grandTrine = detectGrandTrine(aspects, filteredPlanets);
  if (grandTrine && grandTrine.strength >= config.minimalPatternStrength) patterns.push(grandTrine);
  
  const tSquare = detectTSquare(aspects, filteredPlanets);
  if (tSquare && tSquare.strength >= config.minimalPatternStrength) patterns.push(tSquare);
  ...
  if (aspects.length > 0) {
    ...
    summaryParts.push(`${aspects.length} aspects (${hardAspects} hard, ${softAspects} soft)`);
  }
  if (patterns.length > 0) {
    summaryParts.push(`${patterns.length} pattern(s): ${patterns.map(p => p.type).join(', ')}`);
  }
  if (wars.length > 0) {
...
export function getStrongestPlanets(analysis: PatternAnalysis, limit: number = 3): string[] {
...
export function getWeakestPlanets(analysis: PatternAnalysis, limit: number = 3): string[] {
  return Object.values(analysis.strengths)
