...
export function parseInput(
  ...
  for (const line of lines) {
    ...
    let normalized = line.replace(/°|′|'/, " ").replace(/\s+/g, " ").trim();
    ...
    const planetMatch = normalized.match(
      /^(Transit\s+)?([A-Za-z\s\-]+?)(?=\s*[\d])/i
    );
    if (!planetMatch) {
    ...
    const degMinMatch = normalized.match(
      /(\d{1,3}(?:[.,]\d+)?)\s*(?:(\d{1,2})\s*)?/
    );
    if (!degMinMatch) {
    ...
    if (houseMatch) {
      ...
      if (h >= 1 && h <= 12) house = h;
        ...
        errors.push(`House number ${h} out of range (1-12) in line: "${line}"`);
    }

    const absolute = zodiacDegree(sign, degree);
    if (absolute === null) {
      errors.push(
        `Invalid degree ${degree} for sign ${sign} in line: "${line}"`
      );
      continue;
    }
    ...
    if (parsed[planet]) {
      errors.push(
        `Duplicate entry for ${planet} – overwriting previous. Line: "${line}"`
      );
    }
...
export function runAstroReading(
  ...
  const mindResult = scorePillar(
    "mind",
    effectiveNatal,
    effectiveTransits,
    activations
  );
  const soulResult = scorePillar(
    "soul",
    effectiveNatal,
    effectiveTransits,
    activations
  );
  const spiritResult = scorePillar(
    "spirit",
    effectiveNatal,
    effectiveTransits,
    activations
