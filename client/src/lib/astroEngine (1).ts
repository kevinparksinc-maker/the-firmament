...
// HYBRID: Sidereal/Vedic framework + Outer planets (Uranus/Neptune/Pluto)
...
export const PLANET_NAMES = [
  ...
  'Rahu', 'Ketu', 'Uranus', 'Neptune', 'Pluto', 'Asc'
];

export const PLANET_GLYPHS: Record<string, string> = {
  Sun:'☉', Moon:'☽', Mercury:'☿', Venus:'♀', Mars:'♂',
  Jupiter:'♃', Saturn:'♄', Rahu:'☊', Ketu:'☋', 
  Uranus:'⛢', Neptune:'♆', Pluto:'♇', Asc:'↑'
};
...
export const PRIORITY: Record<string, number> = {
  Saturn:5, Jupiter:4, Rahu:3.5, Ketu:3.5, Mars:3,
  Pluto:3.8, Uranus:3.2, Neptune:3.1,
  Sun:2, Venus:2, Mercury:2, Moon:1.5
};
...
export const PLANET_CORE: Record<string, Record<string, string>> = {
  ...
  Uranus: {
    mind:'innovation, sudden insight, electrical thinking, rebellion',
    soul:'freedom, individuality, awakening, breaking patterns',
    spirit:'collective change, breakthrough, liberation'
  },
  Neptune: {
    mind:'imagination, confusion, dream logic, artistic vision',
    soul:'transcendence, sacrifice, compassion, spiritual longing',
    spirit:'mystical union, creative flow, dissolution of ego'
  },
  Pluto: {
    mind:'depth, obsession, psychological insight, investigation',
    soul:'power, transformation, death and rebirth, shadow work',
    spirit:'collective evolution, purging, regeneration'
  }
};
...
export function buildReadingText(result: ReadingResult): string {
  return `
✦ MIND ✦
${result.mind.state}
${result.mind.body}
...
✦ ACTIVATIONS ✦
${result.activations.slice(0, 5).map(a => a.summary).join("\n")}
  `.trim();
}
...
export function parseInput(
  ...
  parsed: Record<string, PlanetPlacement>;
  rawLines: string[];
  errors: string[];
} {
  ...
  for (const line of lines) {
    ...
    // Normalize: replace '°' and '′' with spaces, collapse multiple spaces
    let normalized = line
      .replace(/°|′|'/, ' ')
      .replace(/\s+/g, ' ')
      .trim();
    ...
    // Detect flags
    const rx = /\bRx\b|retrograde/i.test(normalized);
    const combust = /combust/i.test(normalized);
    const cazimi = /cazimi/i.test(normalized);

    // Extract planet name (allow letters, spaces, hyphens) up to first number
    const planetMatch = normalized.match(/^(Transit\s+)?([A-Za-z\s\-]+?)(?=\s*[\d])/i);
    if (!planetMatch) {
      errors.push(`Could not parse planet name in line: "${line}"`);
      continue;
    }
    let planetRaw = planetMatch[2].trim();
    const planet = normalizePlanet(planetRaw);
    if (!planet) {
      errors.push(`Unknown planet "${planetRaw}" in line: "${line}"`);
      continue;
    }

    // Extract degrees and minutes
    // Pattern: number (maybe decimal, maybe with comma) then optional minutes number
    const degMinMatch = normalized.match(/(\d{1,3}(?:[.,]\d+)?)\s*(?:(\d{1,2})\s*)?/);
    if (!degMinMatch) {
      errors.push(`No degree value in line: "${line}"`);
      continue;
    }
    let degree = parseFloat(degMinMatch[1].replace(',', '.'));
    if (isNaN(degree)) {
      errors.push(`Invalid degree number in line: "${line}"`);
      continue;
    }
    if (degMinMatch[2]) {
      const minutes = parseInt(degMinMatch[2], 10);
      if (!isNaN(minutes)) degree += minutes / 60;
    }

    // Extract sign name (must match one of 12 signs)
    ...
    if (!signMatch) {
      errors.push(`No valid zodiac sign in line: "${line}"`);
      continue;
    }
    const sign = titleCase(signMatch[1]);

    // Extract house number (optional, 1-12)
    let house: number | null = null;
    const houseMatch = normalized.match(/(\d{1,2})(?:st|nd|rd|th)?\s*house/i);
    if (houseMatch) {
      const h = parseInt(houseMatch[1], 10);
      if (h >= 1 && h <= 12) house = h;
      else errors.push(`House number ${h} out of range (1-12) in line: "${line}"`);
    }

    const absolute = zodiacDegree(sign, degree);
    if (absolute === null) {
      errors.push(`Invalid degree ${degree} for sign ${sign} in line: "${line}"`);
      continue;
    }

    // Warn if duplicate planet (overwrites silently)
    if (parsed[planet]) {
      errors.push(`Duplicate entry for ${planet} – overwriting previous. Line: "${line}"`);
    }
