...
const SIGN_COLORS: Record<string, string> = {
  Aries: '#ff6644', Taurus: '#88cc44', Gemini: '#ffcc44', Cancer: '#88ccff',
  Leo: '#ffaa22', Virgo: '#aaccaa', Libra: '#ffccaa', Scorpio: '#cc4444',
  Sagittarius: '#cc8844', Capricorn: '#888888', Aquarius: '#4488ff', Pisces: '#8888ff',
};

const PLANET_SYMBOLS: Record<string, string> = {
  Sun: '☉', Moon: '☽', Mercury: '☿', Venus: '♀', Mars: '♂',
  Jupiter: '♃', Saturn: '♄', Uranus: '⛢', Neptune: '♆', Pluto: '♇',
  Rahu: '☊', Ketu: '☋', Asc: 'AC', Dsc: 'DC', MC: 'MC', IC: 'IC',
};

const PLANET_COLORS: Record<string, string> = {
  Sun: '#ffd700', Moon: '#e8e8ff', Mercury: '#aaaaaa', Venus: '#ffccaa',
  Mars: '#ff4444', Jupiter: '#ffaa44', Saturn: '#ddcc88', Uranus: '#88ffee',
  Neptune: '#4488ff', Pluto: '#aa88cc', Rahu: '#88aaff', Ketu: '#ff8888',
};
...
export function ChartWheel({ planets, houseCusps, angles, size = 420 }: ChartWheelProps) {
  const cx = size / 2;
  const cy = size / 2;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
      <svg width={size} height={size} style={{ overflow: 'visible' }}>
        {/* Background */}
        <circle cx={cx} cy={cy} r={outerR} fill="#050810" stroke="#1a2a3a" strokeWidth="1" />
        ...
        {ZODIAC_SIGNS.map((sign, i) => {
          ...
          const symPos = polarToXY(cx, cy, (zodiacR + zodiacInnerR) / 2, midAngle);

          return (
            <g key={sign}>
              <path d={path} fill={color} fillOpacity="0.12" stroke={color} strokeOpacity="0.3" strokeWidth="0.5" />
              <text
                x={symPos.x} y={symPos.y}
                textAnchor="middle" dominantBaseline="central"
                fontSize={size * 0.032} fill={color} fillOpacity="0.9"
        ...
        {/* Outer ring */}
        <circle cx={cx} cy={cy} r={zodiacR} fill="none" stroke="#1a2a3a" strokeWidth="1" />
        <circle cx={cx} cy={cy} r={zodiacInnerR} fill="none" stroke="#1a2a3a" strokeWidth="0.5" />
        <circle cx={cx} cy={cy} r={houseR} fill="none" stroke="#1a2a3a" strokeWidth="0.5" />
        ...
        {houseLines.map(({ angle, inner, outer, cusp }) => {
          ...
          return (
            <g key={cusp.house}>
              ...
              {/* House number */}
              {(() => {
                const numAngle = lonToAngle(ascLon + (cusp.house - 1) * 30 + 15, ascLon);
                const numPos = polarToXY(cx, cy, (houseR + innerR) / 2, numAngle);
                return (
                  <text
                    x={numPos.x} y={numPos.y}
                    textAnchor="middle" dominantBaseline="central"
                    fontSize={size * 0.025} fill="#445566" fillOpacity="0.8"
              ...
              {/* Sign on cusp */}
              {(() => {
                const cuspPos = polarToXY(cx, cy, zodiacInnerR - size * 0.025, angle);
                const sign = houseCusps[cusp.house - 1]?.sign ?? '';
                const color = SIGN_COLORS[sign] ?? '#888';
                return (
                  <text
                    x={cuspPos.x} y={cuspPos.y}
                    textAnchor="middle" dominantBaseline="central"
                    fontSize={size * 0.022} fill={color} fillOpacity="0.9"
                    style={{ fontFamily: 'serif' }}
                  >
                    {cusp.degree}°
                  </text>
        ...
        {/* Inner circle */}
        <circle cx={cx} cy={cy} r={innerR} fill="#030608" stroke="#1a2a3a" strokeWidth="1" />
        ...
        {[
          ...
          return (
            <text
              ...
              x={pos.x} y={pos.y}
              textAnchor="middle" dominantBaseline="central"
              fontSize={size * 0.028} fill={color} fontWeight="bold"
              style={{ fontFamily: "'Cinzel', serif" }}
            >
              {label}
            </text>
        ...
        {planetPositions.map((p, i) => {
          ...
          return (
            <g key={`${p.name}-${i}`}>
              <circle cx={p.pos.x} cy={p.pos.y} r={size * 0.022} fill="#050810" stroke={color} strokeWidth="1" strokeOpacity="0.6" />
              <text
                x={p.pos.x} y={p.pos.y}
                textAnchor="middle" dominantBaseline="central"
                fontSize={size * 0.028} fill={color}
                style={{ fontFamily: 'serif' }}
              >
                {symbol}
              </text>
              {rx && (
                <text
                  x={p.pos.x + size * 0.022} y={p.pos.y - size * 0.018}
                  fontSize={size * 0.018} fill={color} fillOpacity="0.7"
                >
                  {rx}
                </text>
              )}
      ...
      <div style={{ width: '100%', maxWidth: `${size}px` }}>
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: '10px', letterSpacing: '3px', color: 'var(--ember)', marginBottom: '10px', textAlign: 'center' }}>
          HOUSE CUSPS
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4px' }}>
          {houseCusps.map((h) => (
            <div key={h.house} style={{ background: 'var(--surface)', border: '1px solid var(--rim)', borderRadius: '3px', padding: '6px 8px', textAlign: 'center' }}>
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: '9px', color: 'var(--ember)', letterSpacing: '1px' }}>H{h.house}</div>
              <div style={{ fontSize: '12px', color: SIGN_COLORS[h.sign] ?? '#fff', marginTop: '2px' }}>
                {SIGN_SYMBOLS[h.sign]} {h.degree}°{h.minutes}'
              </div>
              <div style={{ fontSize: '10px', color: 'var(--silver-dim)', marginTop: '1px' }}>{h.sign}</div>
            </div>
          ))}
        </div>
      </div>
