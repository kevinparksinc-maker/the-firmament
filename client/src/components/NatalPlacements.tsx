...
function PlanetCard({ p, starMap }: any) {
  ...
  return (
    <div style={{ border: `1px solid ${hasRoyal ? '#c8a050' : 'var(--rim)'}`, borderRadius: '4px', background: 'var(--deep)', overflow: 'hidden', transition: 'border-color 0.3s' }}>
      ...
      <div
        onClick={() => setOpen(o => !o)}
        style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', borderBottom: open ? '1px solid var(--rim)' : 'none', background: 'rgba(255,255,255,0.02)', cursor: 'pointer', userSelect: 'none' }}
        onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.04)'}
        onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}
      >
        <div style={{ fontSize: '20px', width: '28px', textAlign: 'center' }}>{glyph}</div>
        <div style={{ flex: 1 }}>
          ...
          {houseKeywords.length > 0 && (
            <div style={{ display: 'flex', gap: '6px', marginTop: '4px', flexWrap: 'wrap' }}>
              {houseKeywords.map((k, i) => (
                <span key={i} style={{ fontSize: '9px', color: 'var(--silver-dim)', padding: '2px 6px', border: '1px solid var(--rim)', borderRadius: '999px', letterSpacing: '1px' }}>{k}</span>
              ))}
            </div>
          )}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px' }}>
          <div style={{ fontFamily: "'Cinzel', serif", fontSize: '9px', letterSpacing: '2px', color: dignity.color, padding: '3px 8px', border: `1px solid ${dignity.color}`, borderRadius: '999px' }}>
            {dignity.label}
          </div>
          <div style={{ color: 'var(--silver-dim)', fontSize: '11px', transition: 'transform 0.3s', transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>▾</div>
        </div>
      </div>
      ...
      {open && (
        <div>
          {/* Tab bar */}
          <div style={{ display: 'flex', borderBottom: '1px solid var(--rim)', overflowX: 'auto' }}>
            {TABS.map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                style={{
                  background: 'none', border: 'none', borderBottom: activeTab === tab ? '2px solid var(--ember)' : '2px solid transparent',
                  color: activeTab === tab ? 'var(--ember)' : 'var(--silver-dim)',
                  fontFamily: "'Cinzel', serif", fontSize: '9px', letterSpacing: '2px',
                  padding: '10px 14px', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'color 0.2s',
                }}
              >{tab}</button>
            ))}
          </div>
          ...
          <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <>
                {nak && (
                  <Row label="NAKSHATRA" color="var(--ember)">
                    {nak.nakshatra.name} pada {nak.pada} — lord {nak.nakshatra.lord}
                  </Row>
                )}
                <Row label="DECAN" color="var(--ember)">{decanRuler} — {decanFlavor}</Row>
                {sabian && <Row label="SABIAN" color="var(--ember)"><em>"{sabian}"</em></Row>}
                {stars.map((s, si) => (
                  <Row key={si} label={s.star.isRoyal ? '\u2605 ROYAL' : 'FIXED STAR'} color="#c8a050">
                    <span style={{ color: '#c8a050' }}>{s.star.name} (orb {s.orb}°) — {s.star.archetype}</span>
                    <div style={{ fontSize: '12px', color: 'var(--silver-dim)', marginTop: '4px' }}>{s.star.gift}</div>
                  </Row>
                ))}
              </>
            )}
            ...
            {!houseMeaning && activeTab !== 'LAYERS' && (
              <div style={{ fontSize: '13px', color: 'var(--silver-dim)', fontStyle: 'italic' }}>House meaning not available for this placement.</div>
            )}
          </div>
        </div>
      )}
...
function Row({ label, color, children }: any) {
  return (
    <div style={{ display: 'flex', gap: '8px', fontSize: '13px', alignItems: 'flex-start' }}>
      ...
      <span style={{ color: 'var(--silver)', lineHeight: 1.6 }}>{children}</span>
    </div>
  );
}
