...
interface BirthDataFormProps {
  onChartCalculated: (readingText: string, planets: any[], lat: number, lng: number, houseCusps: any[], angles: any) => void;
  disabled?: boolean;
}
...
export function BirthDataForm({ onChartCalculated, disabled }: BirthDataFormProps) {
  ...
  const handleCalculate = async () => {
    ...
    if (!y || !m || !d) { setError('Enter birth year, month, and day.'); return; }
    ...
    try {
      ...
      onChartCalculated(result.readingText, result.planets, la, lo, result.houseCusps ?? [], result.angles ?? { asc: 0, desc: 180, mc: 90, ic: 270 });
    } catch (err: any) {
  ...
  return (
    <div style={{ marginBottom: '16px', border: '1px solid var(--rim)', borderRadius: '4px', overflow: 'hidden' }}>
      {/* Header toggle */}
      <button
      ...
      {expanded && (
        <div style={{ padding: '16px', background: 'rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ fontSize: '12px', color: 'var(--silver-dim)', fontStyle: 'italic', lineHeight: 1.5 }}>
            Enter your birth data to auto-calculate your topocentric sidereal chart — no copy-pasting required.
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
            <div>
              ...
              <input type="number" placeholder="1985" value={year} onChange={e => setYear(e.target.value)} style={inputStyle} min="1900" max="2100" />
            </div>
            <div>
              <label style={labelStyle}>Month</label>
              <input type="number" placeholder="11" value={month} onChange={e => setMonth(e.target.value)} style={inputStyle} min="1" max="12" />
            </div>
            <div>
              <label style={labelStyle}>Day</label>
              <input type="number" placeholder="3" value={day} onChange={e => setDay(e.target.value)} style={inputStyle} min="1" max="31" />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              ...
              <input type="number" placeholder="12" value={hour} onChange={e => setHour(e.target.value)} style={inputStyle} min="0" max="23" />
            </div>
            <div>
              <label style={labelStyle}>Minute</label>
              <input type="number" placeholder="0" value={minute} onChange={e => setMinute(e.target.value)} style={inputStyle} min="0" max="59" />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={city}
              onChange={e => { setCity(e.target.value); setCityResolved(''); }}
              onBlur={handleCityBlur}
              style={inputStyle}
            />
            {cityResolved && (
              <div style={{ fontSize: '11px', color: 'var(--ember)', marginTop: '4px' }}>✓ {cityResolved}</div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              ...
              <input type="number" placeholder="33.7490" value={lat} onChange={e => setLat(e.target.value)} style={inputStyle} step="0.0001" />
            </div>
            <div>
              <label style={labelStyle}>Longitude</label>
              <input type="number" placeholder="-84.3880" value={lng} onChange={e => setLng(e.target.value)} style={inputStyle} step="0.0001" />
            </div>
          </div>

          {error && (
            <div style={{ fontSize: '12px', color: '#e87070', padding: '8px', border: '1px solid #8a3030', borderRadius: '3px' }}>
              {error}
            </div>
          )}
